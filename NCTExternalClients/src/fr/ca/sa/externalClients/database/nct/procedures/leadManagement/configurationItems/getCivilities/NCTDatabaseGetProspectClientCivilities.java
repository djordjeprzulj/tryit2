package fr.ca.sa.externalClients.database.nct.procedures.leadManagement.configurationItems.getCivilities;
...
/**
 * 
 * Store procedure call for retrieving prospect client civilities
 * 
 * @author Sasa Radovanovic
 *
 */
public class NCTDatabaseGetProspectClientCivilities extends AbstractUserLanguageStoreProcedureHandler {

	private static final Logger LOGGER = Logger.getLogger(NCTDatabaseGetProspectClientCivilities.class);

	/**
	 * NCT Database Get Prospect Client Civilities
	 * 
	 * @param userId
	 * 				User ID
	 * 
	 ...
	 * 				Language ID
	 */
	public NCTDatabaseGetProspectClientCivilities(Integer userId, Integer langId) {
		super(userId, langId);
	}

	@Override
	public PreparedStatement buildPreparedStatement(Connection connection) throws SQLException { 
		PreparedStatement ps = connection.prepareStatement("exec spGetProspectClientCivilities ?,?");
		try {
	...
	protected StoreProcedureResult handleResultSet() throws StatementExecutionException {
		...
		try {
			while (this.getResultSet().next()) {
				retList.getItemList().add(new LeadManagementNCTResultItem(this.getResultSet().getInt("CivilityId"),
						this.getResultSet().getString("CivilityName")));
			}
		} catch (SQLException ex) {
			LOGGER.error(ex);
